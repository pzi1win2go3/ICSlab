...
eval(char *cmdline) 
{
    ...
    int inputfd_close = 0;
    int outputfd_close = 0;
    if (tok.infile != NULL)
    {
        inputfd = open(tok.infile, O_RDONLY);
        inputfd_close = 1;
    }
    if (tok.outfile != NULL)
    {
        outputfd = open(tok.outfile, O_WRONLY | O_TRUNC | O_CREAT,
    ...
    /* not build-in command, load and run executable file */
    else
    {
        sigset_t mask;
        sigemptyset(&mask);
