...
#define GET(p)          (*(unsigned long *)(p))          //read word at address p
#define PUT(p, val)     (*(unsigned long *)(p) = (val))  //write word at address p
...
#define HDRP(bp)       ((char *)(bp) - WSIZE * 3)                               //get ptr's header address
#define FTRP(bp)       ((char *)(bp) + GET_SIZE(HDRP(bp)) - WSIZE * 4)              //get ptr's footer address
#define PRED(bp)       ((char *)(bp) - WSIZE * 2)
#define SUCC(bp)       ((char *)(bp) - WSIZE)      
#define NEXT_BLKP(bp)  ((char *)(bp) + GET_SIZE((char *)(bp) - WSIZE * 3))     //next block
#define PREV_BLKP(bp)  ((char *)(bp) - GET_SIZE((char *)(bp) - WSIZE * 4))     //prev block

#define CLASSES_NUM 19
#define INF 0x7FFFFFFF
#define EXTRA (4 * WSIZE)
#define EXTRA_W 4
...
int mm_init(void) 
{
    ...
    if ((heap_listp = mem_sbrk(CLASSES_NUM * WSIZE)) == (void *)-1)
        return -1;
...
void *realloc(void *oldptr, size_t size)
{
    ...
    if(asize < oldsize) oldsize = asize;
...
void mm_checkheap(int verbose) 
{
    ...
    if (verbose == 1)
    {
        ...
        for (int i = 0; i < CLASSES_NUM; i++)
        {
            ...
            {
                char *cur_blk = (char *)GET(heap_listp + i * WSIZE);
                while (1)
                {
                    print_blk(cur_blk);

                    if (GET(SUCC(cur_blk)) == 0)
                        break;
                    cur_blk = (char *)GET(SUCC(cur_blk));
                } 
            }
        }
    }
...
static void *coalesce(void *bp)
{
    ...
    // make cur_blk an independent free block
    destroy_relationship(cur_blk);
...
static void insert(char *cur_blk, size_t asize)
{
    ...
    char *old_first = (char *)GET(heap_listp + class_index * WSIZE);
    ...
    PUT(heap_listp + class_index * WSIZE, (unsigned long)cur_blk);
    PUT(PRED(cur_blk), 0);
    PUT(SUCC(cur_blk), 0);
    ...
    {
        PUT(PRED(old_first), (unsigned long)cur_blk);
        PUT(SUCC(cur_blk), (unsigned long)old_first);
    }
}
...
static inline void destroy_relationship(char *cur_blk)
{
    char *pred = (char *)GET(PRED(cur_blk));
    char *succ = (char *)GET(SUCC(cur_blk));
    ...
    if (pred == NULL)
    {
        ...
        PUT(heap_listp + cur_index * WSIZE, (unsigned long)succ);
    ...
    {
        PUT(SUCC(pred), (unsigned long)succ);

        if (succ != NULL)
        {
            PUT(PRED(succ), (unsigned long)pred);
        }
